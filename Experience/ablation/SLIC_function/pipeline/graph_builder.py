...
def compute_node_features(
    ...
    for sp_id in unique_ids:
        ...
        if image is not None and image.size > 0:
            ...
            # 与图像一致的坐标，避免 map CSV 与图像尺寸不一致时 step2 大量 clip 到边界
...
def _enumerate_patient_map_dirs(dataset_root: Path) -> List[Tuple[str, Path]]:
    """
    列出 (规范化后的 patient_id, map_csv 目录)。

    优先 ``dataset/{patient}/map_csv``；若顶层无含 CSV 的 map_csv，则回退搜索
    ``**/map_csv``（例如 ``dataset/队列/患者/map_csv``）。
    """
    ...
    for d in sorted(dataset_root.iterdir()):
        if not d.is_dir():
            continue
        md = d / "map_csv"
        if md.is_dir() and any(md.glob("*.csv")):
            groups.append((normalize_patient_id(d.name), md))
    if groups:
        return groups

    for md in sorted(dataset_root.rglob("map_csv")):
        if not md.is_dir() or not any(md.glob("*.csv")):
            continue
        parent = md.parent
        if parent == dataset_root:
            continue
        groups.append((normalize_patient_id(parent.name), md))
    return groups
...
def collect_slice_graphs(
    ...
    Returns:
        {patient_id: {slice_id: SliceGraph}}（键为 :func:`~path_utils.normalize_patient_id` 后的 ID）
    """
    from .path_utils import normalize_patient_id

    dataset_root = Path(dataset_root)
    result: Dict[str, Dict[str, SliceGraph]] = {}

    groups = _enumerate_patient_map_dirs(dataset_root)
    if patient_ids is not None:
        want = {normalize_patient_id(p) for p in patient_ids}
        groups = [(pid, md) for pid, md in groups if pid in want]

    for pid, map_dir in groups:
        ...
        for csv_path in sorted(map_dir.glob("*.csv")):
            ...
            if image_dir:
                ...
                img_path = resolve_slide_image(image_dir, pid, slice_id)
            g = build_slice_graph(
                str(csv_path),
                patient_id=pid,
                slice_id=slice_id,
