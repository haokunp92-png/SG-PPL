...
Fig 2 分组仍仅用 bootstrap p 值（见 _classify_pite 默认行为）。
（已移除整批治疗臂对比 KM 汇总图。）
"""
...
# Fig4 瀑布图：|ITE| 百分比绝对值低于该阈值者归入 ITE-neutral（再按符号+p 值区分 preferred/opposed）
WATERFALL_NEUTRAL_ABS_ITE_PCT = 5.0
...
def _classify_pite(results: List[Dict]) -> Dict[str, List[Dict]]:
    """按 ITE 显著性将患者分为 ITE-preferred / ITE-neutral / ITE-opposed。"""
    preferred = []
    neutral = []
    opposed = []
    for r in results:
...
def _plot_fig4_waterfall_ite(results: List[Dict], output_path: str) -> None:
    ...
    legend_elements = [
        ...
        Patch(facecolor="#95a5a6", edgecolor="none", label="ITE-neutral (no significant effect)"),
