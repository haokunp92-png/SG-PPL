...
from .path_utils import normalize_patient_id
...
def load_clinical_csv(csv_path: str) -> Dict[str, Dict[str, Any]]:
    ...
    键已用 :func:`normalize_patient_id` 规范化（与 Step1/4 目录名对齐）。
    ...
    with open(csv_path, "r", encoding="utf-8-sig") as f:
        ...
        fn_clean = [((h or "").lstrip("\ufeff").strip()) for h in fieldnames]
        has_event = "event" in fn_clean
        for row in rows:
            row = {
                ((k or "").lstrip("\ufeff").strip() if isinstance(k, str) else k): v
                for k, v in row.items()
            }
            if "patient_id" not in row or row["patient_id"] is None or str(row["patient_id"]).strip() == "":
                continue
            pid = normalize_patient_id(row["patient_id"])
            result[pid] = {
...
def run_step5_fusion(
    ...
    img_feats = {normalize_patient_id(k): v for k, v in img_feats.items()}
    img_feats_causal = None
    if step4_pkl_causal:
        img_feats_causal = load_step4_feats(step4_pkl_causal)
        img_feats_causal = {
            normalize_patient_id(k): v for k, v in img_feats_causal.items()
        }

    if patient_ids is not None:
        want = {normalize_patient_id(p) for p in patient_ids}
        clinical = {k: v for k, v in clinical.items() if k in want}
        img_feats = {k: v for k, v in img_feats.items() if k in want}
        if img_feats_causal is not None:
            img_feats_causal = {
                k: v for k, v in img_feats_causal.items() if k in want
            }

    # 对齐：取临床与图像的交集
    common = sorted(set(clinical.keys()) & set(img_feats.keys()))
    if not common:
        c_keys = sorted(clinical.keys())
        i_keys = sorted(img_feats.keys())
        raise ValueError(
            "临床数据与图像特征无交集患者，请检查 patient_id 是否一致（Excel 导出 float、"
            "列名 BOM 等已自动规范化）。\n"
            f"  临床 patient_id 共 {len(c_keys)} 例，示例: {c_keys[:12]}\n"
            f"  影像 step4 键共 {len(i_keys)} 例，示例: {i_keys[:12]}"
        )
