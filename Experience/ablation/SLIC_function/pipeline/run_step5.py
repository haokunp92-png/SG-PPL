"""
第五步：临床数据读取与模态对齐 - 运行脚本

读取 clinical.csv，与第四步图像特征对齐融合，划分 train/val/test。
"""
...
def main():
    parser = argparse.ArgumentParser(description="第五步：临床数据读取与模态对齐")
    parser.add_argument(
        "--clinical",
    ...
    # 上游联合图模式一致性检查：避免误把“未做联合图编码”的 step4 特征当作 upstream 输入
