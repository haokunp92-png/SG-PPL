"""
对比实验运行脚本：DeepSurv 基线，报告 C-index。
...
from .deepsurv import DEFAULT_DROPOUT, DEFAULT_HIDDEN, DEFAULT_WEIGHT_DECAY, train_deepsurv
from .report import build_summary, print_c_index, print_dataset_banner, save_results

_NOTE = "关联模型（MLP Cox），不含治疗变量，无法产出 ITE/PITE；仅报告 C-index。"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="对比实验：DeepSurv 基线（多模态治疗无关输入，报告 C-index）"
    )
    parser.add_argument(
        "--step4_output",
    ...
    parser.add_argument(
        "--tag",
        default=None,
        help="实验标识，用于结果文件名（如 slic_deepsurv）；默认按前端目录推断",
    )
    parser.add_argument(
        "--input_mode",
        default="concat",
    ...
    parser.add_argument(
        ...
        help="AdamW 权重衰减（DeepSurv 的 ridge 正则）",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="重复训练次数（种子递增，划分不变）；n=34 的评估集噪声大，>1 可报 mean±std",
    )
    ...
    tag = args.tag or f"deepsurv_{Path(args.step4_output).resolve().parent.name}"

    print_dataset_banner(tag, "DeepSurv", dataset)
